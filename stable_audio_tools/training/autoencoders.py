...
class AutoencoderTrainingWrapper(pl.LightningModule):
    def __init__(
        ...
        if self.autoencoder.out_channels == 2:
            self.sdstft = auraloss.SumAndDifferenceSTFTLoss(
                sample_rate=sample_rate, **stft_loss_args
            )
            self.lrstft = auraloss.MultiResolutionSTFTLoss(
                sample_rate=sample_rate, **stft_loss_args
            )
        else:
        ...
        if self.use_disc:
            if loss_config["discriminator"]["type"] == "oobleck":
                self.discriminator = OobleckDiscriminator(
                ...
                self.discriminator = BigVGANDiscriminator(
                    channels=self.autoencoder.out_channels,
                    sample_rate=sample_rate,
                    **loss_config["discriminator"]["config"],
                )
        ...
        if self.use_disc:
            self.gen_loss_modules += [
                ...
                ValueLoss(
                    ...
                    weight=self.loss_config["discriminator"]["weights"][
                        "feature_matching"
                    ],
        ...
        if self.teacher_model is not None:
            ...
            self.gen_loss_modules += [
                MSELoss(
                    key_a="teacher_latents",
                    key_b="latents",
                    weight=stft_loss_weight,
                    name="latent_distill_loss",
                ...
                AuralossLoss(
                    self.sdstft,
                    input_key="decoded",
                    target_key="teacher_decoded",
                    name="mrstft_loss_distill",
                    weight=stft_loss_weight,
                    decay=stft_loss_decay,
                ...
                AuralossLoss(
                    self.sdstft,
                    target_key="reals",
                    input_key="own_latents_teacher_decoded",
                    name="mrstft_loss_own_latents_teacher",
                    weight=stft_loss_weight,
                    decay=stft_loss_decay,
                ...
                AuralossLoss(
                    self.sdstft,
                    target_key="reals",
                    input_key="teacher_latents_own_decoded",
            ...
            if self.autoencoder.out_channels == 2:
                ...
                self.gen_loss_modules += [
                    AuralossLoss(
                        self.lrstft,
                        target_key="reals_left",
                        input_key="decoded_left",
                        name="stft_loss_left",
                    ...
                    AuralossLoss(
                        self.lrstft,
                        target_key="reals_right",
                        input_key="decoded_right",
        ...
        if "hubert" in loss_config:
            ...
            if hubert_weight > 0:
                hubert_cfg = (
                    ...
                    if "config" in loss_config["hubert"]
                    else dict()
                )
        ...
        if eval_loss_config is not None:
            ...
            if "sisdr" in eval_loss_config:
                self.eval_losses["sisdr"] = auraloss.SISDRLoss(
                    **eval_loss_config["sisdr"]
                )
    ...
    def configure_optimizers(self):
        ...
        if self.use_disc:
            ...
            if (
                "scheduler" in self.optimizer_configs["autoencoder"]
                and "scheduler" in self.optimizer_configs["discriminator"]
            ):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            latents, encoder_info = self.autoencoder.encode(
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.teacher_model is not None:
            with torch.no_grad():
                teacher_latents = self.teacher_model.encode(
                    encoder_input, return_info=False
                )
        ...
        if self.teacher_model is not None:
            with torch.no_grad():
                ...
                own_latents_teacher_decoded = self.teacher_model.decode(
                    latents
                )  # Distilled model's latents decoded by teacher
                teacher_latents_own_decoded = self.autoencoder.decode(
                    teacher_latents
                )  # Teacher's latents decoded by distilled model
        ...
        if self.use_disc:
            if self.warmed_up:
                loss_dis, loss_adv, feature_matching_distance = self.discriminator.loss(
                    reals=reals, fakes=decoded
                )
            else:
                loss_adv = torch.tensor(0.0).to(reals)
                feature_matching_distance = torch.tensor(0.0).to(reals)
    ...
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):
        ...
        try:
            ...
            if module.discriminator is not None:
                ...
                fakes_stft = torch.stft(
                    fold_channels_into_batch(fakes),
                    n_fft=512,
                    hop_length=128,
                    win_length=512,
                    window=window,
                ...
                real_stft = torch.stft(
                    fold_channels_into_batch(demo_reals),
                    n_fft=512,
                    hop_length=128,
                    win_length=512,
                    window=window,
                ...
                loss.backward()
                grads = unfold_channels_from_batch(
                    fakes_stft.grad.detach().abs(), fakes.shape[1]
                )
                log_dict[f"disciminator_sensitivity"] = wandb.Image(
                    tokens_spectrogram_image(
                        grads.mean(dim=1).log10(),
                        title="Discriminator Sensitivity",
                        symmetric=False,
                    )
                )
            ...
            log_image(
                trainer.logger, "embeddings_spec", tokens_spectrogram_image(latents)
            )
            log_image(
                trainer.logger,
                "recon_melspec_left",
...
def create_loss_modules_from_bottleneck(bottleneck, loss_config):
    ...
    if (
        isinstance(bottleneck, VAEBottleneck)
        or isinstance(bottleneck, DACRVQVAEBottleneck)
        or isinstance(bottleneck, RVQVAEBottleneck)
    ):
        try:
    ...
    if isinstance(bottleneck, RVQBottleneck) or isinstance(
        bottleneck, RVQVAEBottleneck
    ):
        quantizer_loss = ValueLoss(
            key="quantizer_loss", weight=1.0, name="quantizer_loss"
        )
        losses.append(quantizer_loss)

    if isinstance(bottleneck, DACRVQBottleneck) or isinstance(
        bottleneck, DACRVQVAEBottleneck
    ):
        codebook_loss = ValueLoss(
            key="vq/codebook_loss", weight=1.0, name="codebook_loss"
        )
        commitment_loss = ValueLoss(
            key="vq/commitment_loss", weight=0.25, name="commitment_loss"
        )
